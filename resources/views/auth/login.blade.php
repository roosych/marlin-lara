    ...
    <div class="card p-4 border-top-left-radius-0 border-top-right-radius-0">
        ...
        <form action="{{route('login')}}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="email" id="username" class="form-control" placeholder="{{__('Эл. адрес')}}" name="email">
                @error('email')
            ...
            <div class="form-group text-left">
                <div class="custom-control custom-checkbox">
                    ...
                    <label class="custom-control-label" for="rememberme" name="remember">{{__('Запомнить меня')}}</label>
                </div>
            </div>
            <button type="submit" class="btn btn-default float-right">{{__('Войти')}}</button>
        </form>
    </div>
    <div class="blankpage-footer text-center">
        {{__('Нет аккаунта?')}} <a href="{{route('register')}}"><strong>{{__('Зарегистрироваться')}}</strong></a>
    </div>
</div>
